...
//            time = Math.max(time, ct);
            ...
            for(int i = 0; i < 4; i++) {
                ...
                } else if(cmap[nx][ny] == -3) {
                    cmap[nx][ny] = nt;
                    q.add(new Point(nx, ny, nt));
//                    if(checkNewVirusSpread(nx, ny)) {
//                        cmap[nx][ny] = nt;
//                        q.add(new Point(nx, ny, nt));
//                    }
//                    else {
//                        cmap[nx][ny] = ct;
//                        q.add(new Point(nx, ny, ct));
//                    }
                } else {
                    if(cmap[nx][ny] > nt) {
                        cmap[nx][ny] = nt;
    ...
    private static boolean checkNewVirusSpread(int x, int y) {
        for(int i = 0; i < 4; i++) {
            int nx = x + ax[i];
            int ny = y + ay[i];
            if(nx < 0 || nx >= N || ny < 0 || ny >= N) continue;
            if(cmap[nx][ny] == -1) return true;
        }
        return false;
    }
    ...
    private static void setting() {
        for(int i = 0; i < N; i++) {
            for(int j = 0; j < N; j++) {
                ...
                else if(map[i][j] == 1) cmap[i][j] = -2;
                else if(map[i][j] == 2) cmap[i][j] = -3;
            }
        }
    }
